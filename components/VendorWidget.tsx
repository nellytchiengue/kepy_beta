    ...
    <div className="w-full max-w-xs flex flex-col items-center gap-4">
      {user ? (
        <div className="w-full text-center p-3 bg-primary/10 rounded-lg border border-primary/20">
          <p className="text-sm font-medium text-slate-600 dark:text-slate-300">{t('vendor_widget.logged_in_as')}</p>
          <p className="text-base font-semibold text-text-light dark:text-text-dark truncate" title={user.email || ''}>{user.email}</p>
        </div>
      ) : (
        <div className="w-full">
          <label htmlFor="vendorId" className="sr-only">{t('vendor_widget.vendor_id_placeholder')}</label>
          <input
            id="vendorId"
            type="text"
            value={vendorId}
            onChange={handleInputChange}
            placeholder={t('vendor_widget.vendor_id_placeholder')}
          ...
          />
          <p id="vendorId-help" className="text-xs text-slate-500 mt-1">{t('vendor_widget.id_help_text')}</p>
        </div>
      )}
      ...
      >
        {t('vendor_widget.cta_enter_sale')}
      </a>
      ...
      <a
        ...
        onClick={(e) => isVendorIdEmpty && e.preventDefault()}
      >
        {t('vendor_widget.cta_my_dashboard')}
      </a>
    </div>
  );
};
